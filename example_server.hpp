...
struct glfw_state {
    glfw_state(float yaw = 15.0, float pitch = 15.0, float roll = 0) : yaw(yaw), pitch(pitch), roll(roll), last_x(0.0), last_y(0.0), last_z(0.0),
        ml(false), offset_x(0.f), offset_y(0.f), offset_z(0.f), tex() {}
    double yaw;
    double pitch;
    double roll;
    double last_x;
...
void draw_pointcloud(float width, float height, glfw_state& app_state, rs2::points& points)
{
    ...
    glRotated(app_state.pitch, 1, 0, 0);// key button control  --rotation
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, 0x812D); // GL_CLAMP_TO_EDGE
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, 0x812D); // GL_CLAMP_TO_EDGE
    glBegin(GL_POINTS);
