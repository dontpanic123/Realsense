...
public:
    ...
    operator bool()
    {
        ...
        glViewport(-0.5, -0.5, _width, _height);
...
struct glfw_state {
    glfw_state(float yaw = 15.0, float pitch = 15.0, float roll = 15.0) : yaw(yaw), pitch(pitch),roll(roll), last_x(0.0), last_y(0.0),last_z(0.0),
        ml(false), offset_x(2.f), offset_y(2.f),offset_z(2.f), tex() {}
    double yaw;
    double pitch;
    double roll;
    double last_x;
...
void draw_pointcloud(float width, float height, glfw_state& app_state, rs2::points& points)
{
    ...
    glTranslatef(+0.5f + app_state.offset_x * 0.05f, 0, 0);
